...
from app.audit.chain import append_audit_entry, verify_audit_chain_integrity, get_external_anchor, get_external_anchor_history

def test_hmac_hash_chain_integrity_and_anchoring(db):
    ...
    initial_history_len = len(get_external_anchor_history())
    ...
    # Check external anchor updated and appended (never overwritten)
    anchor = get_external_anchor()
    assert anchor is not None
    assert anchor["sequence_no"] == entry.sequence_no
    assert anchor["entry_hash"] == entry.entry_hash

    # Check append-only history grew
    history = get_external_anchor_history()
